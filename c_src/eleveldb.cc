...
/**
 * async_write:  originally designed to always send write requests to
 *               leveldb via a worker thread (to avoid scheduler collapse).
 *               Updated Nov. 2016 to write directly to leveldb if blocking
 *               conditions known to be minimized.
 *
 *     returns:  atom "ok" if write completed successfully on caller's thread
 *               "caller_ref" generated by eleveldb.erl if write request posted
 *                  to worker pool.  caller should await message with "ok" or error
 *               error atom if write failed on caller's thread or posting to worker pool
 */
